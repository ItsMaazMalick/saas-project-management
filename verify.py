...
ACME_MGR = r.get("tokens", {}).get("access", "")
...
ACME_EMP = r.get("tokens", {}).get("access", "")
...
ok("Login response has access token", "access" in r.get("tokens", {}), True)
ok("Login response has refresh token", "refresh" in r.get("tokens", {}), True)
...
REFRESH_TOKEN = r.get("tokens", {}).get("refresh", "")

s, r = req("POST", "/auth/token/refresh/", {"refresh": REFRESH_TOKEN})
ok("Refresh token → 200", s, 200)
ok("Refresh returns new access token", "access" in r, True)
...
# Logout
s, r = req("POST", "/auth/logout/", {"refresh": REFRESH_TOKEN}, token=ACME)
ok("Logout → 200", s, 200)

# Blacklisted token should fail
s, r = req("POST", "/auth/token/refresh/", {"refresh": REFRESH_TOKEN})
ok("Blacklisted refresh token rejected → 401", s, 401)

# Re-login Acme admin (get fresh token)
s, r = req("POST", "/auth/login/", {"email": "admin@acme.com", "password": "Admin1234!"})
ACME = r.get("tokens", {}).get("access", "")
...
EMP_ID = next((u["id"] for u in users if u["email"] == "emp@acme.com"), None)
...
ok("Project owner defaults to requester", r.get("owner", {}).get("email") == "admin@acme.com", True)
...
ok("Project owner is manager", r.get("owner", {}).get("email") == "manager@acme.com", True)
PROJ2_ID = r.get("id")
...
s, r = req("POST", f"/tasks/{TASK_ID}/update_status/", {"status": "in_progress"}, token=ACME_EMP)
...
# Beta cannot see Acme users
s, r = req("GET", "/users/", token=BETA)
ok("Beta sees 0 Acme users", r.get("count", 0), 0)
...
if logs:
    ...
    ok("Audit log has user_id field", "user_id" in log or "user" in log, True)
else:
    ok("Audit log has action field", False, True)
    ok("Audit log has resource_type field", False, True)
    ok("Audit log has timestamp field", False, True)
    ok("Audit log has user_id field", False, True)
...
# Rate limit headers (or just check requests pass)
...
deleted_ids = [p["id"] for p in r.get("results", r if isinstance(r, list) else [])]
...
# Note: We can't verify Celery actually processed the task without a worker
# but we can verify the tasks module is importable and the endpoints work
...
# DB indexes (compound index on tasks)
# Can only verify by running a query; we just confirm tasks are filterable
encoded = urllib.parse.urlencode({"status": "todo"})
s, r = req("GET", f"/tasks/?{encoded}", token=ACME)
